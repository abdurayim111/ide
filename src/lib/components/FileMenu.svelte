...
<div
	...
	use:melt={$menu}
>
	{#each [[Plus, 'New File', '/new', null, 'Ctrl+Alt+N'], [Download, 'Download File', null, onDownloadFile, 'Ctrl+S'], [Files, 'Clone File', `${window.location.origin}${window.location.pathname}/copy`, null, 'Ctrl+Alt+C'], [Cog, 'Settings', null, onOpenSettings, null]] as [Icon, text, href, onclick, shortcut] (text)}
		<a
			use:melt={$item}
			{href}
			{onclick}
			target="_blank"
			class="group flex w-full cursor-pointer items-center justify-between px-4 py-2 text-sm focus:outline-none"
			class:text-gray-800={theme === 'light' || theme === 'huacat-pink'}
		...
		>
			...
			{#if shortcut}
				<span class="ml-4 text-xs text-gray-400 opacity-75">{shortcut}</span>
			{/if}
		</a>
	{/each}
</div>
